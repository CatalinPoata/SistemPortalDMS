...
namespace API_PORTAL.Controllers.Auth
{
    [ApiController]
    [Route("api/auth")]
    public sealed class AuthController : ControllerBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (IsPublicPortalRequest() && user.role != Role.Citizen)
            {
                return Unauthorized(InvalidCredentials());
            }
        ...
        public async Task<ActionResult<TokenResponse>> VerifyTotpLogin(
            ...
            CancellationToken cancellationToken)
        {
            if (!totpService.TryReadLoginChallenge(request.Challenge, out var userId))
            {
                return Unauthorized(InvalidCredentials());
            }
            ...
            if (user is null || userTotp is null || user.role != Role.Admin ||
            ...
            var verified = totpService.TryVerify(
                totpService.UnprotectSecret(userTotp.protected_secret),
                request.Code, userTotp.last_used_counter, out var counter);
            ...
            CancellationToken cancellationToken)
        {
            ...
            var existing = await db.UserTotps.SingleOrDefaultAsync(
                item => item.user_id == user.id, cancellationToken);
            if (existing?.is_enabled == true)
            {
                return Conflict(new ProblemDetails
                {
                    Status = StatusCodes.Status409Conflict,
                    Title = "TOTP este deja activ",
                    Detail = "Dezactivează autentificarea în doi pași înainte de a o configura din nou."
                });
            }
            ...
            if (existing is null)
            {
                existing = new UserTotp { id = Guid.NewGuid(), user_id = user.id };
            ...
            return Ok(new TotpSetupResponse(enrollment.Secret, enrollment.OtpAuthUri));
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (userTotp is null)
            {
                return UnprocessableEntity(TotpSetupUnavailable());
            }

            if (!totpService.TryVerify(
                    totpService.UnprotectSecret(userTotp.protected_secret),
                    request.Code, null, out var counter))
            {
                return UnprocessableEntity(InvalidTotpCode());
            }
            ...
            userTotp.last_used_counter = counter;
            userTotp.recovery_code_hashes = JsonDocument.Parse(JsonSerializer.Serialize(
                recoveryCodes.Select(totpService.HashRecoveryCode).ToArray()));
            ...
            CancellationToken cancellationToken)
        {
            ...
            var user = await db.Users.SingleOrDefaultAsync(
                item => item.id == userId.Value && item.role == Role.Admin,
                cancellationToken);
            var userTotp = await db.UserTotps.SingleOrDefaultAsync(
                item => item.user_id == userId.Value && item.is_enabled,
                cancellationToken);
            ...
            if (user is null || userTotp is null ||
                passwordHasher.VerifyHashedPassword(user, user.password_hash,
            ...
            var verified = totpService.TryVerify(
                totpService.UnprotectSecret(userTotp.protected_secret),
                request.Code, userTotp.last_used_counter, out _);
            if (!verified) verified = TryConsumeRecoveryCode(userTotp, request.Code);
            if (!verified) return UnprocessableEntity(InvalidTotpCode());
    ...
    CancellationToken cancellationToken)
        {
            ...
            var issuedTokens = jwtTokenService.Create(user);
        ...
        [HttpGet("me")]
        [Authorize]
        public Task<ActionResult<CurrentUserResponse>> Me(
            CancellationToken cancellationToken)
        {
            return ReadProfileAsync(
                requireAdmin: false,
                cancellationToken);
        }
        ...
        [HttpGet("admin-me")]
        [Authorize(Roles = nameof(Role.Admin))]
        public Task<ActionResult<CurrentUserResponse>> AdminMe(
            CancellationToken cancellationToken)
        {
            return ReadProfileAsync(
                requireAdmin: true,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var message =
                "Dacă datele sunt valide, contul a fost creat și trebuie confirmat.";
            ...
            var user = new User
            {
                ...
                national_id = Normalize(request.NationalId),
        ...
        {
            var issuedTokens = jwtTokenService.Create(user);
            db.RefreshTokens.Add(issuedTokens.RefreshTokenEntity);
            await db.SaveChangesAsync(cancellationToken);

            SetRefreshCookie(
                issuedTokens.RefreshToken,
                issuedTokens.RefreshTokenEntity.expires_at);
        ...
        private bool IsPublicPortalRequest() =>
            string.Equals(
                Request.Headers["X-Portal-Frontend"],
                "APP-PUB",
        ...
        private static ProblemDetails TotpSetupUnavailable() => new()
        {
            Status = StatusCodes.Status422UnprocessableEntity,
            Title = "Configurare TOTP indisponibilă",
            Detail = "Începe mai întâi configurarea autentificării în doi pași."
        };

        private static ProblemDetails InvalidTotpCode() => new()
        {
            Status = StatusCodes.Status422UnprocessableEntity,
            Title = "Cod TOTP invalid",
            Detail = "Codul este invalid, expirat sau a fost deja folosit."
        };
            ...
            DateTimeOffset expiresAt)
        {
            Response.Cookies.Append(
                AuthCookieOptions.RefreshTokenName,
                refreshToken,
                AuthCookieOptions.Create(expiresAt));
        }
        ...
        private async Task<ActionResult<CurrentUserResponse>> ReadProfileAsync(
            bool requireAdmin,
            CancellationToken cancellationToken)
        {
            var userId = GetCurrentUserId();
