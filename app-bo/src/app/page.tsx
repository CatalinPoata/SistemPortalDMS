          ...
          >
            <div className="space-y-2">
              ...
              <p>
                ...
                codurile de recuperare salvate la activare.
              </p>
              <p className="text-sm text-slate-600">
                Cererea expiră la{" "}
                {new Date(auth.expiresAt).toLocaleTimeString("ro-RO")}.
              </p>
            </div>
        ...
        {auth.status === "authenticated" && (
          <div className="space-y-6">
            <section className="rounded-2xl bg-gradient-to-r from-blue-700 to-indigo-700 p-6 text-white shadow-lg sm:p-8">
              <p className="text-sm font-medium text-blue-100">Sesiune activă</p>
              <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-semibold">Bun venit, {auth.user.fullName}</h2>
                  <p className="mt-1 text-blue-100">{auth.user.email} · {roleLabels[auth.user.role]}</p>
                </div>
                <Link href="/servicii" className="rounded-lg bg-white px-4 py-2 font-medium text-blue-800 shadow-sm transition hover:bg-blue-50">
            ...
            <section>
              <div className="mb-3 flex items-center justify-between gap-4">
                <h2 className="text-lg font-semibold">Module administrative</h2>
                <p className="text-sm text-slate-600">Alege zona pe care vrei să o administrezi.</p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Link href="/servicii" className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md"><span className="text-2xl">▤</span><h3 className="mt-3 font-semibold">Servicii</h3><p className="mt-1 text-sm text-slate-600">Formulare, publicare și versiuni.</p></Link>
                <Link href="/cereri" className="rounded-xl border border-indigo-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md"><span className="text-2xl">⌁</span><h3 className="mt-3 font-semibold">Cereri</h3><p className="mt-1 text-sm text-slate-600">Urmărește depunerile cetățenilor.</p></Link>
                <Link href="/outbox" className="rounded-xl border border-amber-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-md"><span className="text-2xl">↗</span><h3 className="mt-3 font-semibold">Outbox</h3><p className="mt-1 text-sm text-slate-600">Monitorizează livrările către DMS.</p></Link>
                <Link href="/articole" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md"><span className="text-2xl">✎</span><h3 className="mt-3 font-semibold">Articole</h3><p className="mt-1 text-sm text-slate-600">Conținutul public al portalului.</p></Link>
                <Link href="/programari" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md"><span className="text-2xl">◷</span><h3 className="mt-3 font-semibold">Programări</h3><p className="mt-1 text-sm text-slate-600">Tipuri, intervale și capacități.</p></Link>
                <Link href="/rapoarte" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md"><span className="text-2xl">▥</span><h3 className="mt-3 font-semibold">Rapoarte</h3><p className="mt-1 text-sm text-slate-600">Preview și export PDF sau CSV.</p></Link>
              </div>
            </section>

            <section className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <Link href="/registre" className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-950">Registre publice</Link>
              <Link href="/chestionare" className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-950">Chestionare</Link>
              <Link href="/utilizatori" className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-950">Utilizatori</Link>
