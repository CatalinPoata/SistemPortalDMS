...
{
    public sealed record RenderedReportHtml(
        string Html,
        bool ShowPageNumbers);

    public static class DmsReportHtmlDocument
    {
        private static readonly CultureInfo RomanianCulture =
        ...
        public static RenderedReportHtml Create(
            ...
            IQueryCollection query)
        {
            ...
            var title = ExpandTemplate(
                layout.Title,
                definition.Parameters,
                query) ?? report.name;

            var subtitle = ExpandTemplate(
                layout.Subtitle,
                definition.Parameters,
                query);

            var body = definition.RenderMode == "record"
                ? BuildRecord(preview)
                : BuildTable(preview);

            return new RenderedReportHtml(
                BuildHtml(
                    title,
                    subtitle,
                    layout.Orientation,
        ...
        private static string BuildHtml(
            ...
            string body)
        {
            var safeOrientation = orientation == "landscape"
                ? "landscape"
                : "portrait";
            ...
            <head>
                ...
                <style>
                    ...
                    * { box-sizing: border-box; }
                    ...
                    .record {
                        display: grid;
                        grid-template-columns: minmax(35%, 42%) 1fr;
                        margin: 0;
        ...
        private static string BuildTotals(
        ...
        private static string FormatValue(
            object? value,
            ReportPreviewColumnResponse column)
        {
            if (value is null)
            {
                return "—";
            ...
            return value switch
            {
                ...
                IFormattable formattable => FormatFormattable(
                    formattable,
                    column.Format),
            ...
            string? format)
        {
            ...
            {
                return value.ToString("dd.MM.yyyy", RomanianCulture);
            }
        }
        ...
        private static string FormatFormattable(
            IFormattable value,
            string? format)
        {
            try
            {
                return value.ToString(format, RomanianCulture)
                    ?? string.Empty;
            }
            catch (FormatException)
            {
                return value.ToString(null, RomanianCulture)
                    ?? string.Empty;
            }
        }
            ...
            IQueryCollection query)
        {
            ...
            return PlaceholderPattern.Replace(
                ...
                match =>
                {
                    ...
                    if (!declaredParameters.Contains(name) ||
                        !query.TryGetValue(name, out var suppliedValue) ||
                        suppliedValue.Count != 1)
                    {
                        return "—";
                    }

                    return suppliedValue[0] ?? "—";
                });
        }
    }
}
