...
async function withAuthLock<T>(
  ...
  return navigator.locks.request("dms-auth-session", action);
}
...
async function readUserLocked(): Promise<CurrentUser> {
  ...
  if (user.role !== "Clerk" && user.role !== "Admin") {
    throw new ApiError(403, {
      detail: "Acest cont nu are acces la aplicația DMS.",
    });
  }
...
export function initializeAuth() {
  ...
  if (typeof BroadcastChannel !== "undefined") {
    channel = new BroadcastChannel("dms-auth-events");
  ...
  return withAuthLock(async () => {
    ...
    if (state.status !== "authenticated") {
      throw new ApiError(401, {
        detail: "Autentifică-te pentru a continua.",
      });
    }
