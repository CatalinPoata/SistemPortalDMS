...
export default function TotpSecurityPage() {
  ...
  async function beginSetup() {
    await run(async () => {
      ...
      setMessage(
        "Adaugă secretul în aplicația de autentificare, apoi confirmă codul curent.",
      );
    });
  }
  ...
  async function enable(event: FormEvent<HTMLFormElement>) {
    ...
    await run(async () => {
      ...
      setMessage(
        "Autentificarea în doi pași este activă. Salvează recovery codes acum; nu vor mai fi afișate.",
      );
    });
  }
  ...
  if (auth.status === "loading") {
    return <p className="p-6">Se verifică sesiunea…</p>;
  }

  if (auth.status === "totp") {
    return <p className="p-6">Completează verificarea TOTP pentru a accesa securitatea contului.</p>;
  }

  if (auth.status === "error") {
    return <div className="p-6"><ApiErrorMessage error={auth.error} /></div>;
  }

  if (auth.status === "anonymous") {
    return <p className="p-6">Autentifică-te pentru a gestiona securitatea contului.</p>;
  }

  const accountLabel =
    auth.user.role === "Admin" ? "administrator" : "funcționar";

  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-6">
      <div className="mx-auto max-w-3xl space-y-6">
        <Link href="/" className="text-sm font-medium text-blue-700 hover:underline">
          ← Înapoi la panoul DMS
        </Link>
        ...
        <header>
          ...
          <p className="mt-3 text-slate-700">
            Protejează contul de {accountLabel} cu o aplicație compatibilă TOTP,
        ...
        {enrollment && (
          <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-semibold">
              1. Adaugă contul în aplicația de autentificare
            </h2>
            ...
            <a
              ...
              className="inline-block break-all text-blue-700 underline"
            >
              Deschide aplicația de autentificare
            </a>
            <div>
              <label className="text-sm font-medium" htmlFor="totp-secret">
                Secret manual (afișat o singură dată)
              </label>
              <input
                id="totp-secret"
                readOnly
            ...
            <form onSubmit={enable} className="space-y-4 border-t border-slate-200 pt-5">
              ...
              <div>
                <label className="text-sm font-medium" htmlFor="enable-totp-code">
                  Cod cu 6 cifre
                </label>
                <input
                  id="enable-totp-code"
              ...
              <button type="submit" disabled={busy} className={primaryButton}>
                {busy ? "Se verifică…" : "Activează TOTP"}
              </button>
            </form>
          </section>
        )}
        ...
        {recoveryCodes && (
          <section className="rounded-xl border-2 border-amber-300 bg-amber-50 p-6">
            <h2 className="text-xl font-semibold text-amber-950">
              Recovery codes — salvează-le acum
            </h2>
            <p className="mt-2 text-amber-950">
              Fiecare cod funcționează o singură dată. Nu va mai putea fi recuperat din aplicație.
            </p>
            ...
            <button
              type="button"
              className="mt-5 rounded-lg border border-amber-700 px-4 py-2 font-medium text-amber-950 hover:bg-amber-100"
        ...
        <section className="rounded-xl border border-red-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-slate-700">
            Este necesară parola curentă și un cod TOTP sau recovery code nefolosit.
          </p>
          <form onSubmit={disable} className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label htmlFor="disable-totp-code" className="text-sm font-medium">
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-fit rounded-lg bg-red-700 px-4 py-2 font-medium text-white hover:bg-red-800 disabled:opacity-60"
            >
